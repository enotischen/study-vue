...
(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    _showOrHideCrumbsList: function (disableAnimation) {
      var that = this;
      this.remainingSpaceToDisplayCrumbs = this.$element.width(); // nav width
      this.nbCrumbDisplayed = 0;
      this.totalCrumbsWidth = 0;
    ...
    _showOrHideCrumb: function ($crumb, $nextCrumb, crumbPosition, disableAnimation) {
      ...
      var that = this;
      this.remainingSpaceToDisplayCrumbs -= ($crumb.data('width')+(1*rem()));
      ...
      function showCrumbWithOrWithoutAnimation() {
        ...
        if (that.lastNbCrumbDisplayed < (that.nbCrumbDisplayed + 1) && that.options.animation.activated && !disableAnimation) {
          $crumb.width(0);
          $crumb.animate({width: $crumb.data('width')+(1*rem())}, that.options.animation.speed, function () {
      ...
      function enableEllipsis($crumb) {
        $crumb.css({
          'width': (that.remainingSpaceToDisplayCrumbs + $crumb.data('width')-(1*rem())) + 'px'
        });
        $crumb.addClass('text-truncate');
      }
      ...
      function disableEllipsis($crumb) {
        ...
        $crumb.removeClass('text-truncate');
      }
    },
    ...
    _showOrHideCrumbsListOnWindowResize: function () {
      ...
      $(window).resize(function () {
        var rcrumbWidth = that.$element.find('ol.breadcrumb').width(); // resize nav width
        ...
        if (rcrumbWidth >= that.totalCrumbsWidth && that.$lastCrumb.hasClass('text-truncate')) {
...
"undefined"!=typeof document&&function(e,t){var n=e.createElement("style");if(e.getElementsByTagName("head")[0].appendChild(n),n.styleSheet)n.styleSheet.disabled||(n.styleSheet.cssText=t);else try{n.innerHTML=t}catch(e){n.innerText=t}}(document,"@charset \"UTF-8\";.rcrumbs li{display:none}");
